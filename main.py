...
thyme = args.thyme #'/home/tuur/Desktop/Datasets/THYME/THYME/THYME-clean/BOTH/'
if args.load_document_structure:
	with open(args.load_document_structure, 'rb') as f:
		thyme_document_structure = pickle.load(f)
else:
	thyme_document_structure = read_thyme(thyme,regex='.*Temp.*', max_documents=max_num_documents, closure=closure, lowercase=bool(args.lowercase), conflate_digits=bool(args.conflate_digits),ctakes_out_dir=args.ctakes_out_dir, pos=args.pos)
...
if structured_prediction:
	...
	SP = StructuredPerceptron(labels, dims,averaged=averaged,loss_augmented_training=loss_augmented_training,balance=balance,structured_feature_handler=feature_extractor_struct, regularization=regularization, regularization_term=regularization_term)
