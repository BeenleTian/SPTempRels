	...
	def extract(self, X, Y=None, update=True):
		...
		if self.TLINK_argument_bigrams:
			...
			if Y:
				...
				#features_expressions.update({'TLINK_arg_bigrams:' + str(((t1,a1,l1), (t2,a2,l2))) :[] for l1 in self.labels_ee for l2 in self.labels_ee for t1 in ['TIMEX3','EVENT','DOCTIME','SECTIONTIME'] for t2 in ['TIMEX3','EVENT','DOCTIME','SECTIONTIME'] if l1 != 'no_label' and l2!='no_label'})
...
class DocTimeRelFeatureHandler(FeatureHandler):
	...
	def extract(self, entity, document, update=True):
		...
		if self.closest_verb:
			...
			#features.update({'closest_verb_left:' + closest_left.get_string():1})
			#features.update({'closest_verb_left_pos:' + closest_left.get_string():1})
			
			closest_right, dr = document.tokenization.first_right_verb(entity)
			
			#features.update({'closest_verb_right:' + closest_right.get_string():1})
			#features.update({'closest_verb_right_pos:' + closest_right.pos:1})
...
class TLinkFeatureHandler(FeatureHandler):
	...
	token_distance = False # non-boolean features are bad for the structured perceptron	
...
#			features.update({'rev:'+str(entityPair.e2.get_span()[0] < entityPair.e1.get_span()[0])+'num_events_ib:' +str(num_events):1,'rev:'+str(entityPair.e2.get_span()[0] < entityPair.e1.get_span()[0])+'num_timex3s_ib:' +str(num_timex3):1})
